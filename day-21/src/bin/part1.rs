...
fn build_path(mut p: Point, map: &Vec<Vec<char>>, mut path: Vec<Point>) -> Vec<Point>{

    // check north
    if check_north(&p, map) {
        loop {
            let p_length = path.len();
            let p_w = get_point(&p, 'n');
            if p_w == path[path.len()-1].clone() { break }
            path = build_path(p_w, map, path.clone());
            if p_length == path.len() { break }
        }
    }

    // check south
    if check_south(&p, map) {
        loop {
            let p_length = path.len();
            let p_w = get_point(&p, 's');
            if p_w == path[path.len()-1].clone() { break }
            path = build_path(p_w, map, path.clone());
            if p_length == path.len() { break }
        }
